...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private bool m_ConstantVerticalFollowing = false; //When set to true, the camera always follow vertically the player. If false, it only follows when the player is on the ground
        ...
        private GameObject targetObj; //Detects if the target object is grounded or in the air.
        ...
        {
            bool isTargetGrounded = targetObj.GetComponent<PlatformerCharacter2D>().m_Grounded;
            ...
            if (isTargetGrounded || m_ConstantVerticalFollowing)
            {
                targetPosition = m_Target.position;
            } 
            else
            {
                targetPosition = new Vector3(m_Target.position.x, transform.position.y, 0);
            }
